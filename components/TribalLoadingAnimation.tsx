// TribalLoadingAnimation.tsx
// Animación épica Canvas 2D para el loading del análisis LLM
// Partículas que se condensan en un tornado tribal
...
// Colores de Tribu Impulsa
const COLORS = [
  '#6161FF', // Púrpura principal
  '#00CA72', // Verde success
  '#FFCC00', // Amarillo
  '#FB275D', // Rojo
  '#00D4FF', // Cyan
];

const TribalLoadingAnimation: React.FC<TribalLoadingProps> = ({ 
  isLoading, 
  duration = 4500 
}) => {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      const gradient = ctx.createRadialGradient(
        centerX, centerY, 0,
        centerX, centerY, glowRadius * (0.5 + progress * 0.5)
      );
      ...
      particlesRef.current.forEach((p, i) => {
        ...
        if (progress < 0.3) {
          ...
          // Add slight rotation around center
          const dx = p.x - centerX;
          const dy = p.y - centerY;
          const dist = Math.sqrt(dx * dx + dy * dy);
          const rotationSpeed = 0.002;
          p.x = centerX + dx * Math.cos(rotationSpeed) - dy * Math.sin(rotationSpeed);
          p.y = centerY + dx * Math.sin(rotationSpeed) + dy * Math.cos(rotationSpeed);
          
        } else if (progress < 0.6) {
          // CONVERGING PHASE: Tornado spiral
          ...
          // Add vertical oscillation (tornado effect)
          p.y += Math.sin(elapsed * 0.01 + i) * 2 * tornadoProgress;
          
        } else {
          // CRYSTALLIZING PHASE: Converge to center
          const crystalProgress = (progress - 0.6) / 0.4;
          
          // Target: geometric crystal shape
          ...
          // Pulsing effect
          const pulse = Math.sin(elapsed * 0.01 + i * 0.1) * 3 * (1 - crystalProgress);
          p.x += Math.cos(crystalAngle) * pulse;
          p.y += Math.sin(crystalAngle) * pulse;
        }
        ...
        if (p.trail.length > 1 && progress > 0.3) {
          ...
          ctx.strokeStyle = p.color.replace(')', `, ${0.1})`).replace('rgb', 'rgba').replace('#', '');
        ...
        particleGlow.addColorStop(0, p.color + Math.floor(p.alpha * 255).toString(16).padStart(2, '0'));
      ...
      // Draw central crystal in final phase
      if (progress > 0.7) {
    ...
    <div className="relative w-full rounded-2xl overflow-hidden" style={{ height: '220px' }}>
      ...
      <div className="absolute top-3 right-3 flex gap-1.5">
        <div className={`w-1.5 h-1.5 rounded-full transition-all duration-300 ${
          phase === 'chaos' ? 'bg-[#FFCC00] scale-150' : 'bg-white/20'
        }`} />
        <div className={`w-1.5 h-1.5 rounded-full transition-all duration-300 ${
          phase === 'converging' ? 'bg-[#6161FF] scale-150' : 'bg-white/20'
        }`} />
        <div className={`w-1.5 h-1.5 rounded-full transition-all duration-300 ${
          phase === 'crystallizing' || phase === 'complete' ? 'bg-[#00CA72] scale-150' : 'bg-white/20'
        }`} />
      </div>
    </div>
  );
