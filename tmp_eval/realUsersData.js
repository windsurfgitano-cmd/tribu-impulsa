...
exports.diagnoseUsers = exports.setUserStatus = exports.updateUserInFirebase = exports.deleteUser = exports.getTotalUsersCount = exports.registerNewUser = exports.emailExists = exports.syncUsersFromFirebase = exports.forceReloadRealUsers = exports.migrateUsersToFirebase = exports.changeUserPassword = exports.markFirstLoginComplete = exports.validateCredentials = exports.getUserFromFirebaseByEmail = exports.getUserByEmail = exports.loadRealUsers = exports.REAL_USERS = exports.UNIVERSAL_PASSWORD = void 0;
// Contraseña universal para todos los usuarios registrados
exports.UNIVERSAL_PASSWORD = 'TRIBU2026';
// Generar avatar con iniciales (Instagram bloquea hotlinking)
// Colores vibrantes basados en el nombre para variedad visual
const getAvatarUrl = (name, _instagram) => {
...
const loadRealUsers = () => {
    ...
    const realEmails = exports.REAL_USERS.map(u => u.email.toLowerCase());
    const existingRealUsers = existingUsers.filter((u) => realEmails.includes(u.email.toLowerCase()));
    if (existingRealUsers.length >= exports.REAL_USERS.length) {
        console.log('✅ Usuarios reales ya cargados');
        return;
    }
    // Limpiar usuarios anteriores y cargar los nuevos
    const usersWithIds = exports.REAL_USERS.map((user, index) => ({
        ...user,
        id: `real_user_${index + 1}`,
        createdAt: new Date().toISOString(),
        password: exports.UNIVERSAL_PASSWORD,
    ...
    console.log(`✅ ${exports.REAL_USERS.length} usuarios REALES cargados con avatares de Instagram`);
};
exports.loadRealUsers = loadRealUsers;
...
const getUserFromFirebaseByEmail = async (email) => {
    ...
    try {
        ...
        const userProfile = {
            ...
            createdAt: ((_c = (_b = (_a = userData.createdAt) === null || _a === void 0 ? void 0 : _a.toDate) === null || _b === void 0 ? void 0 : _b.call(_a)) === null || _c === void 0 ? void 0 : _c.toISOString()) || new Date().toISOString(),
            surveyCompleted: (_d = userData.surveyCompleted) !== null && _d !== void 0 ? _d : true,
            tribeAssigned: (_e = userData.tribeAssigned) !== null && _e !== void 0 ? _e : false,
            // Campos de matching
            ...
            // Campos adicionales
            firstLogin: (_f = userData.firstLogin) !== null && _f !== void 0 ? _f : false,
            password: userData.password || exports.UNIVERSAL_PASSWORD
        };
        // Sincronizar a localStorage para futuras sesiones
        const users = JSON.parse(localStorage.getItem('tribu_users') || '[]');
        const existingIndex = users.findIndex((u) => u.id === userId || u.email.toLowerCase() === email.toLowerCase());
        if (existingIndex >= 0) {
...
exports.getUserFromFirebaseByEmail = getUserFromFirebaseByEmail;
...
const validateCredentials = (email, password) => {
    ...
    if (password === exports.UNIVERSAL_PASSWORD || password === user.password) {
        return user;
    }
    return null;
};
...
const markFirstLoginComplete = (userId) => {
    const users = JSON.parse(localStorage.getItem('tribu_users') || '[]');
...
const migrateUsersToFirebase = async () => {
    try {
        const { getFirestoreInstance } = await Promise.resolve().then(() => require('./firebaseService'));
        const { doc, setDoc, getDoc, collection } = await Promise.resolve().then(() => require('firebase/firestore'));
        const db = getFirestoreInstance();
        if (!db) {
        ...
        let migrated = 0;
        let existing = 0;
        for (let i = 0; i < exports.REAL_USERS.length; i++) {
            const user = exports.REAL_USERS[i];
...
const forceReloadRealUsers = async () => {
...
exports.forceReloadRealUsers = forceReloadRealUsers;
// Cargar usuarios desde Firebase
const loadUsersFromFirebase = async () => {
    try {
        const { getFirestoreInstance } = await Promise.resolve().then(() => require('./firebaseService'));
        const { collection, getDocs } = await Promise.resolve().then(() => require('firebase/firestore'));
        const db = getFirestoreInstance();
        if (!db)
            return [];
        ...
        snapshot.forEach(doc => {
            ...
            users.push({
                ...
                password: data.password || exports.UNIVERSAL_PASSWORD,
...
exports.syncUsersFromFirebase = syncUsersFromFirebase;
...
const registerNewUser = async (userData) => {
    ...
    if ((0, exports.emailExists)(userData.email)) {
    ...
    try {
        const { getFirestoreInstance } = await Promise.resolve().then(() => require('./firebaseService'));
        const { doc, setDoc } = await Promise.resolve().then(() => require('firebase/firestore'));
        const db = getFirestoreInstance();
        if (db) {
...
const deleteUser = async (userId) => {
    try {
        ...
        const { getFirestoreInstance } = await Promise.resolve().then(() => require('./firebaseService'));
        const { doc, deleteDoc } = await Promise.resolve().then(() => require('firebase/firestore'));
        const db = getFirestoreInstance();
        if (db) {
...
const updateUserInFirebase = async (userId, updates) => {
    try {
        ...
        // Actualizar Firebase
        const { getFirestoreInstance } = await Promise.resolve().then(() => require('./firebaseService'));
        const { doc, updateDoc, serverTimestamp } = await Promise.resolve().then(() => require('firebase/firestore'));
        const db = getFirestoreInstance();
        if (db) {
...
const setUserStatus = async (userId, status) => {
    return await (0, exports.updateUserInFirebase)(userId, { status });
};
...
const diagnoseUsers = async () => {
    ...
    const baseEmails = exports.REAL_USERS.map(u => u.email.toLowerCase());
    // Usuarios nuevos (no están en la base de 108)
    const nuevos = localUsers
        .filter((u) => !baseEmails.includes((u.email || '').toLowerCase()))
    ...
    const result = {
        ...
        base: exports.REAL_USERS.length,
        nuevos
    };
...
exports.diagnoseUsers = diagnoseUsers;
exports.default = exports.REAL_USERS;
